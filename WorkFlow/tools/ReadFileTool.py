...
def read_file(file_path: str, workspace_path: str = None) -> Dict[str, Any]:
    ...
    """
    logger = get_logger()
    logger.info(f"read_file tool invoked - file_path: '{file_path}', workspace_path: '{workspace_path}'")
    ...
    if file_path_normalized.endswith(".txt"):
        logger.debug(f"File path already has .txt extension, not appending")
        full_file_path = workspace_dir / file_path_normalized
    else:
        logger.debug(f"Appending .txt extension to file path")
    ...
    try:
        ...
        if not str(resolved_file_path).startswith(str(resolved_workspace)):
            error_msg = f"Security violation: File path attempts to access outside workspace directory"
            logger.error(f"{error_msg} - File: {resolved_file_path}, Workspace: {resolved_workspace}")
            raise ValueError(error_msg)
        ...
        logger.error(f"{error_msg} - Original error: {type(e).__name__}: {e}", exc_info=True)
        raise ValueError(error_msg)
    ...
    if not full_file_path.exists():
        ...
        logger.debug(f"File path components - workspace: {workspace_dir}, normalized_path: {file_path_normalized}")
        return {
            "success": False,
            "file_path": file_path,
            "content": None,
    ...
    try:
        ...
        logger.debug(f"File statistics - Size: {file_size} bytes, Content length: {content_length} chars, Lines: {lines}")
        ...
        logger.debug(f"Permission error details - File exists: {full_file_path.exists()}, Is file: {full_file_path.is_file() if full_file_path.exists() else 'N/A'}")
        return {
            "success": False,
            "file_path": file_path,
            "content": None,
        ...
        logger.error(f"{error_msg} - File: {full_file_path}")
        logger.debug(f"IO error details - Error type: {type(e).__name__}, Error code: {getattr(e, 'errno', 'N/A')}")
        return {
            "success": False,
            "file_path": file_path,
            "content": None,
