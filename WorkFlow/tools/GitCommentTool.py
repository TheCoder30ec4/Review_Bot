...
def format_comment(code_snippet: str, comment: str, impact: ImpactLevel, 
                   current_code: str = None, suggested_code: str = None,
...
def post_code_review_comment(
    ...
    """
    logger = get_logger()
    logger.info(f"post_code_review_comment tool invoked - PR #{pull_request_number}, file: {file_path}, impact: {impact}")
    ...
    if impact not in ["OK", "Medium", "Critical"]:
        error_msg = f"Invalid impact level: {impact}. Must be one of: OK, Medium, Critical"
        logger.error(f"Validation failed: {error_msg}")
        raise ValueError(error_msg)
    ...
    logger.debug(f"Input validation passed - repo: {repo_link}, PR: {pull_request_number}, file: {file_path}")
    ...
    if not token:
        ...
        if token:
            logger.warning("Using GIT_TOKEN instead of GIT_WRITE_TOKEN. Consider using GIT_WRITE_TOKEN for write operations.")
    ...
    formatted_comment = format_comment(code_snippet, comment, impact, current_code, suggested_code, file_path)
    logger.debug(f"Comment formatted successfully ({len(formatted_comment)} characters)")
    ...
    try:
        ...
        logger.debug(f"Comment details - Line: {comment_line or f'{comment_start_line}-{comment_end_line}'}, Side: {side}")
        ...
        try:
            ...
            if not target_file:
                logger.debug(f"Exact match failed for '{file_path}', trying case-insensitive match...")
                for file in pr_files:
                    if file.filename.lower() == file_path.lower():
                        target_file = file
                        actual_file_path = file.filename
                        logger.info(f"Found file with case-insensitive match: {file.filename}")
                        break
            
            # If still not found, try matching just the filename
            if not target_file:
                logger.debug(f"Case-insensitive match failed, trying filename-only match...")
                file_path_basename = Path(file_path).name
                for file in pr_files:
                    if Path(file.filename).name == file_path_basename:
                        target_file = file
                        actual_file_path = file.filename
                        logger.info(f"Found file with filename match: {file.filename} (requested: {file_path})")
                        break
            
            # Log available files if still not found
            if not target_file:
                available_files = [f.filename for f in pr_files]
                logger.warning(f"File '{file_path}' not found in PR. Available files: {available_files[:10]}...")
            
            if target_file and target_file.patch:
                target_line_num = comment_line if comment_line is not None else comment_end_line
                ...
                if target_line_num:
                    ...
                    for i, patch_line in enumerate(patch_lines):
                        ...
                        hunk_match = re.search(r'^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@', patch_line)
                        if hunk_match:
                            new_start = int(hunk_match.group(2))
                            new_line_counter = new_start - 1  # Reset counter for new hunk
                        ...
                        if patch_line.startswith('+') and side.upper() == "RIGHT":
                            ...
                            new_line_counter += 1
                            if side.upper() == "RIGHT" and new_line_counter == target_line_num:
                                position = current_position
                                use_review_comment = True
                                break
                    ...
                    if not use_review_comment and target_line_num:
                        ...
                        use_review_comment = True
                        logger.warning(f"Using approximate position {position} for line {target_line_num}")
                
                logger.debug(f"Calculated position from diff: {position}")
            else:
                logger.warning(f"Could not find file {file_path} in PR files or file has no patch")
        except Exception as e:
        ...
        if not target_file:
            ...
            logger.warning(f"File '{file_path}' not found in PR files. Posting as general PR comment instead.")
            ...
            payload = {
                "body": formatted_comment_with_context
            }
            logger.info(f"Posting as general PR comment (file not in PR diff)")
        else:
            ...
            if position is None:
                ...
                logger.warning(f"Position calculation unavailable, using approximate position: {position}")
            ...
            logger.info(f"Posting as review comment with position: {position}, side: {side.upper()}, path: {actual_file_path}")
        ...
        if response.status_code == 201:
            ...
            return {
                ...
                "impact": impact,
                "file_path": file_path,
                "line_number": comment_line or f"{comment_start_line}-{comment_end_line}"
            }
            ...
            return {
                ...
                "impact": impact,
                "file_path": file_path,
                "line_number": comment_line or (f"{comment_start_line}-{comment_end_line}" if start_line else None)
            }
        else:
            error_data = response.json() if response.text else {}
            error_msg = error_data.get("message", f"GitHub API returned status {response.status_code}")
            detailed_error = f"GitHub API returned status {response.status_code}: {error_msg}"
            logger.error(detailed_error)
            logger.debug(f"Full API response: {response.text}")
        ...
        logger.error(error_msg, exc_info=True)
        logger.debug(f"Failed comment details - File: {file_path}, Line: {comment_line or f'{comment_start_line}-{comment_end_line}'}")
