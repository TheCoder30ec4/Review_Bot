...
def should_ignore_file(file_path: str, logger=None) -> bool:
    ...
    ignore_patterns = [
        ...
        '.env', '.env.local', '.env.production', 'pyproject.toml',
        'package-lock.json', 'yarn.lock', '.gitignore', '.DS_Store',
        'dist/', 'build/', '.venv', 'venv/', '.idea', '.vscode',
        '*.log', '.pytest_cache', '.mypy_cache', '.coverage', 'htmlcov/',
    ]
...
def parse_and_save_diff_files(diff_content: str, output_dir: str = "Output/diff_files", logger=None) -> tuple[List[str], List[str], List[str]]:
    ...
    logger.info(f"Saved {len(saved_files)} file diffs, ignored {len(ignored_files)} files")
...
def get_pull_request(
    ...
    """
    logger = get_logger()
    logger.info(f"Starting PR fetch process for PR #{pull_request_number} from {repo_link}")
    ...
    saved_files, ignored_files, saved_workspace_files = parse_and_save_diff_files(response.text, logger=logger)
    ...
    logger.info(f"Process completed successfully. Total files: {result['total_files']}, Saved: {result['saved_count']}, Ignored: {result['ignored_count']}")
    return result
