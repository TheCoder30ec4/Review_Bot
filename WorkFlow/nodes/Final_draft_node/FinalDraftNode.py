...
Format the comment using Markdown with clear sections, headers, and bullet points for readability on GitHub."""
        ...
        *This summary was automatically generated by the Code Review Bot.*"""
        ...
        # Ensure temporary diff directory for this run is always cleaned up
        try:
            temp_path = Path(run_temp_dir)
            if temp_path.exists():
                shutil.rmtree(temp_path)
                logger.info(
                    f"Cleaned up temporary diff files for this run at: {temp_path}"
                )
        except Exception as cleanup_err:
            logger.warning(
                f"Failed to clean temporary diff folder {run_temp_dir}: {cleanup_err}"
            )
