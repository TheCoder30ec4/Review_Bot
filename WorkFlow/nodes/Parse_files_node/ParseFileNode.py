...
from WorkFlow.PromptLibrary.Prompts import PARSE_FILES_SYSTEM_PROMPT, get_parse_files_invocation_prompt
from WorkFlow.utils.logger import get_logger


def ParseFileNode(fetch_state: FetchState, global_state: Global_State) -> tuple[Global_State, ParseState]:
    """
    ...
    try:
        ...
        for file_path in parse_state.SelectedFilePath:
            ...
            if workspace_file.exists():
                ...
                logger.warning(f"✗ Selected file does not exist in workspace: {file_path}")
        ...
        for invalid_file in invalid_selected:
            parse_state.SkippedFiles.append((invalid_file, "File not found in workspace directory"))
        
        # Update parse_state with validated files only
        parse_state.SelectedFilePath = validated_selected
        
        if invalid_selected:
            logger.warning(f"Filtered out {len(invalid_selected)} invalid selected files that don't exist in workspace")
