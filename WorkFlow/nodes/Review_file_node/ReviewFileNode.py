...
from WorkFlow.PromptLibrary.Prompts import REVIEW_FILE_SYSTEM_PROMPT, get_review_file_invocation_prompt
from WorkFlow.tools.ReadFileTool import read_file_tool
from WorkFlow.utils.logger import get_logger
from WorkFlow.utils.create_agent_with_tools import Agent
...
def find_relevant_files(
    ...
    same_dir_files = [
        ...
        if Path(f).parent == file_dir and f != file_path
    ]
    
    # Strategy 2: Files with similar names (e.g., same base name with different extensions)
    similar_name_files = [
        f for f in all_files 
        if Path(f).stem == file_name and f != file_path
    ]
        ...
        try:
            ...
            if result["success"]:
                ...
                if rel_file in same_dir_files:
                    ...
                    relevance_reason = f"Similar filename suggests related functionality or complementary code"
                elif rel_file in related_files:
                    if 'controller' in file_path.lower() and 'service' in rel_file.lower():
                        relevance_reason = f"Service file used by the controller being reviewed"
                    elif 'service' in file_path.lower() and 'model' in rel_file.lower():
                        relevance_reason = f"Model/entity file used by the service being reviewed"
                    else:
                        relevance_reason = f"Related file in the same module/package"
...
def ReviewFileNode(
    ...
    try:
        ...
        logger.info(f"ReviewFileNode completed for {file_path}. Comment posting will be handled by Flow.py after reflexion.")
