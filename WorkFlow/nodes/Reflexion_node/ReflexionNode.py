...
def parse_failed_generation(error_message: str, logger) -> Optional["ReflexionState"]:
    ...
    try:
        ...
        match = re.search(
            r"'failed_generation':\s*'(.+?)'\s*\}\s*\}", error_message, re.DOTALL
        )
        if not match:
        ...
        logger.info(
            f"🔄 Recovered structured output from failed_generation (confidence: {data.get('Confidence', 'N/A')})"
        )
...
def ReflexionNode(
    review_state: ReviewState, file_content: str, pr_context: Dict[str, Any]
...
Return your validation in the ReflexionState format."""
        ...
        while retry_count < max_retries:
            try:
                ...
                if (
                    ...
                    or "attempted to call tool" in error_str
                ):
                    logger.warning(
                        "Tool call mismatch error - attempting to recover data from failed_generation..."
                    )
                    recovered_state = parse_failed_generation(error_str, logger)
                    if recovered_state:
                        reflexion_state = recovered_state
                        logger.info(
                            f"✅ Successfully recovered ReflexionState from error (confidence: {reflexion_state.Confidence})"
                        )
                        break
                    else:
                        logger.warning(
                            "Could not recover data from failed_generation, will retry..."
                        )
                ...
                if "429" in error_str or "rate_limit" in error_str.lower():
                    ...
                    if retry_count < max_retries:
                        ...
                        logger.warning(
                            f"Rate limit error. Waiting {wait_time:.1f}s before retry {retry_count}/{max_retries}..."
                        )
                    ...
                    if retry_count < max_retries:
                        logger.warning(
                            f"LLM error, retrying ({retry_count}/{max_retries}): {error_str[:200]}..."
                        )
        ...
        if not reflexion_state.IsValid and reflexion_state.ImprovedReviewState:
            logger.info("Reflexion found issues, using improved review state")
        ...
        if reflexion_state.ValidationIssues:
            logger.warning(
                f"Validation issues found: {', '.join(reflexion_state.ValidationIssues)}"
            )

        return (
            reflexion_state.IsValid
            or (reflexion_state.ImprovedReviewState is not None),
            final_review_state,
