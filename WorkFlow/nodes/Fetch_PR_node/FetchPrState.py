...
class PrRequestState(BaseModel):
    ...
    Title: str = Field(
        ...
        examples=["Add authentication feature", "Fix bug in API endpoint", "Update dependencies"]
    )
    ...
    Description: str = Field(
        ...
        examples=["This PR adds user authentication functionality", "Fixes the bug where API returns 500 error", ""]
    )
...
class FetchState(BaseModel):
    ...
    WorkSpacePath: str = Field(
        ...
        examples=["Output/diff_files", "/home/user/project/Output/diff_files", "Output/diff_files/"]
    )
