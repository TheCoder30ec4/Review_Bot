...
from WorkFlow.State import CommentMetadata, Global_State, intial_state
from WorkFlow.utils.logger import get_logger
from WorkFlow.utils.memory_manager import get_memory_manager


def programmatic_fix_review_state(
    review_state, file_path: str, file_content: str, logger
) -> ReviewState:
    """
    ...
    fixed_diff_code = (
        ...
        if review_state.DiffCode
        else extract_code_block(file_content)
    )
    fixed_current_code = (
        review_state.CurrentCode if review_state.CurrentCode else fixed_diff_code
    )
    fixed_suggested_code = (
        review_state.SuggestedCode
        if review_state.SuggestedCode
        else f"// TODO: Review and fix the following code:\n{fixed_current_code}"
    )
    fixed_what_improved = (
        review_state.WhatNeedsToBeImproved
        if review_state.WhatNeedsToBeImproved
        else "Code review required - please review the highlighted code for potential improvements."
    )
    fixed_prompt = (
        review_state.PromptForAI
        if review_state.PromptForAI
        else "Review this code section and suggest improvements for production readiness."
    )
    fixed_criticality = (
        review_state.CriticalityStatus
        if review_state.CriticalityStatus in ["OK", "Medium", "Critical"]
        else "Medium"
    )
...
def fetch_pr_node(state: WorkflowState) -> WorkflowState:
    ...
    session = memory_manager.load_session(
        repo_link=initial.PullRequestLink, pr_number=initial.PullRequestNum
    )

    if session:
        logger.info(
            f"Resumed session: {session.session_id} ({session.total_comments_posted} comments posted)"
        )
...
def review_files_node(state: WorkflowState) -> WorkflowState:
    ...
    memory_manager = get_memory_manager()
    session = memory_manager.load_session(
        repo_link=initial_state.PullRequestLink, pr_number=initial_state.PullRequestNum
    )
    ...
    for file_path in selected_files:
        ...
        if file_review_count >= MAX_COMMENTS_PER_FILE:
            logger.warning(
                f"File {file_path} has been reviewed {file_review_count} times. Skipping to prevent looping."
            )
            continue
        ...
        file_content = (
            file_result.get("content", "") if file_result.get("success") else ""
        )
        ...
        if review_state.CriticalityStatus in ["Medium", "Critical"]:
            ...
            while reflexion_retry_count <= MAX_REFLEXION_RETRIES:
                ...
                is_valid, validated_review_state, confidence, validation_issues = (
                    ReflexionNode(
                ...
                if is_valid and confidence >= MIN_CONFIDENCE_THRESHOLD:
                    ...
                    logger.info(
                        f"✅ Reflexion approved review for {file_path} (confidence: {confidence:.2f})"
                    )
                    break
                ...
                logger.warning(
                    f"❌ Reflexion rejected review (attempt {reflexion_retry_count}/{MAX_REFLEXION_RETRIES + 1}) - confidence: {confidence:.2f}"
                )
                ...
                if reflexion_retry_count > MAX_REFLEXION_RETRIES:
                    ...
                    logger.warning(
                        f"⚠️ All {MAX_REFLEXION_RETRIES} retries exhausted. Attempting programmatic fix for {file_path}..."
                    )
                    ...
                    if (
                        is_valid_fixed or fixed_confidence >= 0.3
                    ):  # Lower threshold for programmatic fix
                        review_state = validated_fixed_state
                        reflexion_approved = True
                        logger.info(
                            f"✅ Programmatic fix approved for {file_path} (confidence: {fixed_confidence:.2f})"
                        )
                    else:
                        # Last resort: use the programmatically fixed state anyway
                        logger.warning(
                            f"⚠️ Programmatic fix validation failed, but proceeding with best-effort review for {file_path}"
                        )
                        reflexion_approved = (
                            True  # Force approval to ensure we don't skip
                        )
                    break

                # Retry: Call ReviewFileNode again with validation issues as additional context
                logger.info(
                    f"🔄 Retrying review generation for {file_path} with validation feedback (attempt {reflexion_retry_count + 1}/{MAX_REFLEXION_RETRIES + 1})..."
                )
            ...
            if not reflexion_approved:
                ...
                logger.warning(
                    f"⚠️ Final fallback: applying programmatic fix for {file_path}"
                )
                review_state = programmatic_fix_review_state(
                    review_state, file_path, file_content, logger
                )
                reflexion_approved = True
            ...
            if memory_manager.check_duplicate_comment(
                session, file_path, issue_summary
            ):
            ...
            for posted_comment in current_global_state.PostedComments:
                if (
                    posted_comment.file_path == file_path
                    and posted_comment.diff_code_hash == diff_code_hash
                ):
                    logger.warning(
                        f"Duplicate comment detected in current session for {file_path}. Skipping."
                    )
                    is_duplicate = True
                    break
            ...
            if not reflexion_approved:
                logger.warning(
                    f"🚫 SECURITY: Attempted to post unapproved comment for {file_path}. Blocking post."
                )
                continue

            # ✅ REFLEXION APPROVED: Post the comment
            logger.info(
                f"📝 Posting reflexion-approved comment for {file_path} (Criticality: {review_state.CriticalityStatus})"
            )
            ...
            try:
                ...
                if comment_result["success"]:
                    ...
                    current_global_state.FileReviewCounts[file_path] = (
                        current_global_state.FileReviewCounts.get(file_path, 0) + 1
                    )
                    ...
                    logger.error(
                        f"❌ Failed to post comment: {comment_result.get('error')}"
                    )
        ...
        while (
            iteration < max_iterations and current_file_comments < MAX_COMMENTS_PER_FILE
        ):
            iteration += 1
            ...
            if next_review_state.CriticalityStatus in ["Medium", "Critical"]:
                ...
                while (
                    not additional_reflexion_approved
                    and additional_retry_count <= MAX_REFLEXION_RETRIES
                ):
                    is_valid, validated_next_state, confidence, validation_issues = (
                        ReflexionNode(
                    ...
                    additional_reflexion_approved = (
                        is_valid and confidence >= MIN_CONFIDENCE_THRESHOLD
                    )

                    if additional_reflexion_approved:
                        next_review_state = validated_next_state
                        logger.info(
                            f"✅ Additional comment approved by reflexion for {file_path} (confidence: {confidence:.2f})"
                        )
                        break
                    ...
                    logger.warning(
                        f"⚠️ Additional review validation failed (attempt {additional_retry_count}/{MAX_REFLEXION_RETRIES}). Issues: {validation_issues}"
                    )

                    if additional_retry_count > MAX_REFLEXION_RETRIES:
                        # 🔧 PROGRAMMATIC FIX for additional comments
                        logger.warning(
                            f"⚠️ All retries exhausted for additional comment. Applying programmatic fix for {file_path}..."
                        )
                        ...
                        if is_valid_fixed or fixed_confidence >= 0.3:
                            ...
                            logger.info(
                                f"✅ Programmatic fix approved for additional comment (confidence: {fixed_confidence:.2f})"
                            )
                        else:
                            ...
                            additional_reflexion_approved = True
                            logger.warning(
                                "⚠️ Using best-effort programmatic fix for additional comment"
                            )
                        break

                    # Retry: Call ReviewFileNode again with validation issues as feedback
                    logger.info(
                        f"🔄 Retrying additional review generation for {file_path} with validation feedback (attempt {additional_retry_count + 1}/{MAX_REFLEXION_RETRIES + 1})..."
                    )
                ...
                if not additional_reflexion_approved:
                    ...
                    logger.warning(
                        "⚠️ Final fallback for additional comment: applying programmatic fix"
                    )
                    next_review_state = programmatic_fix_review_state(
                        current_additional_review, file_path, file_content, logger
                    )
                    additional_reflexion_approved = True

                # Check for duplicates
                diff_code_hash = hashlib.md5(
                    next_review_state.DiffCode.encode()
                ).hexdigest()
                ...
                if memory_manager.check_duplicate_comment(
                    ...
                    logger.warning(
                        "Duplicate additional comment detected. Stopping iterations."
                    )
                    break
                ...
                if not additional_reflexion_approved:
                    logger.warning(
                        f"🚫 SECURITY: Attempted to post unapproved additional comment for {file_path}. Blocking post."
                    )
                    break

                logger.info(
                    f"📝 Posting reflexion-approved additional comment {iteration} for {file_path}"
                )
                ...
                try:
                    ...
                    if comment_result["success"]:
                        ...
                        logger.error(
                            f"❌ Failed to post additional comment: {comment_result.get('error')}"
                        )
                        break
                except Exception as e:
                    logger.error(
                        f"❌ Error posting additional comment: {e}", exc_info=True
                    )
                    break
        ...
        if iteration >= max_iterations:
            logger.warning(
                f"Reached max iterations ({max_iterations}) for {file_path}."
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"   Files in Structure: {len([f for f in result['fetch_state'].PrRequest.FileStructure if f.startswith('> *')])}"
        )
