...
def validate_code_snippet(
    code_snippet: str, file_extension: Optional[str] = None
) -> Tuple[bool, str]:
    """
    ...
    if len(first_chars) > 0 and marker_count / len(first_chars) > 0.5:
        return (
            False,
            "Code snippet contains too many diff markers (not properly cleaned)",
        )
    ...
    if file_extension:
        ...
        if ext in ["py", "python"]:
            ...
            if (
                "def " in code_snippet
                or "class " in code_snippet
                or "import " in code_snippet
            ):
            ...
            if (
                "function " in code_snippet
                or "const " in code_snippet
                or "=>" in code_snippet
            ):
