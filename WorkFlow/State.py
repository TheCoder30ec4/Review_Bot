...
class intial_state(BaseModel):
    ...
    PullRequestNum: int = Field(
        description="The pull request number. Example: 2, 123, 456", examples=[2, 123]
    )
...
class Global_State(BaseModel):
    ...
    RelaventContext: List[str] = Field(
        ...
        examples=[
            ["Authentication logic in lines 10-25", "This function handles user login"],
    ...
    SessionId: Optional[str] = Field(
        description="Session ID for memory tracking across review runs.", default=None
    )
